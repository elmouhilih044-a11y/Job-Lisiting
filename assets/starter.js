...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderFavoriteJobs = () => {
        ...
        if (favoriteJobs.length === 0) {
            favoriteJobsContainer.innerHTML = '<p>Pas de favoris pour le moment </p>';
            return;
        }

        favoriteJobsContainer.innerHTML = favoriteJobs
            .map(job => createJobCardHTML(jcreateJobCardHTML))
            .join('');

    };
    const attachFavoriteListeners = () => {
    document.querySelectorAll('.job-card__favorite-btn').forEach(btn => {
    ...
    const createJobCardHTML = (job) => {
       ...
       <button class="job-card__favorite-btn ${favoriteClass}" data-id="${id}">
    ${favoriteIcon}
</button>
    ...
    const applyAllFilters = () => {
        const combinedFilters = [...userProfile.skills, ...manualFilters];
        searchInput.addEventListener('input', () => {
            const term_search = searchInput.value;
            console.log(term_search);
            const results = allJobs.filter(job =>
                job.company.toUpperCase().includes(term_search.toUpperCase()) ||
                job.position.toUpperCase().includes(term_search.toUpperCase()) ||
                job.level.toUpperCase().includes(term_search.toUpperCase())
            );
